...
var md2Tree = TokenParser.create(function (methods, rgx, utils) {
    ...
    methods.start = function (content) {
        ...
        var par, level, tmp, last, prelast;
                        ...
                        if (par.type === 'head') {
                            if (level > par.level) {
                                ...
                                var firstMatch = findParentChain(function (node) {
                                    return node.type === 'head' && node.level <= level;
                                }, 'first');
                                ...
                                if (firstMatch) {
                                    ...
                                    par = undefined;
                                    t.appendTo('head', undefined, {id: id});
                                }
                            }
                    ...
                    while (tmp) {
                        if (tmp.type === 'list') {
                            if (tmp.level === level) {
                                tmp = tmp.parent;
                                if (!tmp) {
                                    t.appendTo('list', undefined, {id: id});
                                }
                            } else {
